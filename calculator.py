...
def calculate():
    ...
    if not is_safe_expression(expr):
        safe_set("Invalid input")
        return
    

    if re.search(r"/\s*0(\.0*)?(\s|$|[\+\-\*/%])", expr):
        safe_set("Cannot divide by 0")
        add_history(expr, "Error: Divide by 0")
        expression = ""
        return
...
def add_history(expr, res):
    entry = f"{expr} = {res}"
    if not history_list or history_list[-1] != entry:
...
def show_history():
    ...
    tk.Button(hist_win, text="Clear History", font=("Arial", 12),
              bg="#ffdad6", fg="#a33",
              command=lambda: [clear_history(hist_text), refresh_history()]).pack(pady=5)
